...
class YoutubeLiveStream:
    def __init__(self, url, output_dir, cookie={}, video_id=None, max_video_quality=None):
        self.url = url
        self.max_video_quality = max_video_quality

        self.video_info = {}
        self.video_info['id'] = util.get_video_id(url) if not video_id else video_id 
...
We assume a failed download attempt.\nLast segment available was {seg}.")
    ...
    def update_json(self):
        json = util.get_json(self.url, self.cookie)
        self.json = json
        if not json:
    ...
    def populate_info(self):
        ...
        self.video_info['scheduled_time'] = util.get_scheduled_time(self.json.get('playabilityStatus', {}))
    ...
    def update_metadata(self):
        ...
        if self.video_info.get('video_itag'):
            for k, v in util.video_height_ranking.items():
    ...
    def update_status(self):
        ...
        if status == 'LIVE_STREAM_OFFLINE':
            self.status |= Status.OFFLINE
            sched_time = util.get_scheduled_time(playabilityStatus)
...
{playabilityStatus.get('reason', 'No reason found')}. Sub-reason: {subreason}")
    ...
    def update_download_urls(self):
        video_quality = util.get_best_quality(self.json, "video", self.max_video_quality)
        audio_quality = util.get_best_quality(self.json, "audio")

        if video_quality:
            self.video_base_url = util.get_base_url(self.json, video_quality)
            self.video_info['video_itag'] = video_quality
        if audio_quality:
            self.audio_base_url = util.get_base_url(self.json, audio_quality)
        ...
        while True:
            try:
                ...
                with closing(urlopen(video_segment_url)) as in_stream:
                    ...
                    if not util.write_to_file(in_stream, video_segment_filename)\
                        and status == 204\
                ...
                with closing(urlopen(audio_segment_url)) as in_stream:
                    util.write_to_file(in_stream, audio_segment_filename)
